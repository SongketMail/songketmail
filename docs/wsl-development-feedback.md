...
│  - Executes sequential multi-distro container test       │
│  - Captures failure logs, exit codes, and diff analysis │
│  - Serializes report to data/jules_telemetry.json       │
└────────────────────────────┬────────────────────────────┘
                             │
                             ├──────────────────────────────┐
                             ▼                              ▼
...
+-----------------------------------------------------------+
                             ...
                             v
+-----------------------------------------------------------+
| 4. Multi-OS test matrix runs sequentially inside Podman;  |
...
## 📋 Shared Feedback Telemetry Schema

The JSON payload serialized to `data/jules_telemetry.json` adheres to the following structured format:
  ...
  "results": [
    {
      ...
      "status": "SUCCESS",
      "exit_code": 0,
      "logs": "Container started and ran successfully...",
      "diff": "Image state: present"
    }
  ]
}
```
...
- **Jules CLI**: The tool invoking `jules remote new` is wrapped inside local query loops to pull updated task targets or prompt updates.
